...
2. 如果mean(x)不是全部数据的mean的时候，std(y)也不是的时候，Normalize后的数据分布满足下面的关系 $$ \begin{align*} &new_mean = \frac{mean-x}{y}&， mean为原数据的均值，x为传入的均值x \ &new_std = \frac{std}{y} &，y为传入的标准差y\ \end{align*} $$
...
[![img](https://github.com/SpringMagnolia/NLPnote/raw/master/images/1.2/softmax.png)](https://github.com/SpringMagnolia/NLPnote/blob/master/images/1.2/softmax.png)

假如softmax之前的输出结果是`2.3, 4.1, 5.6`,那么经过softmax之后的结果是多少呢？ $$ Y1 = \frac{e^{2.3}}{e^{2.3}+e^{4.1}+e^{5.6}} \ Y2 = \frac{e^{4.1}}{e^{2.3}+e^{4.1}+e^{5.6}} \ Y3 = \frac{e^{5.6}}{e^{2.3}+e^{4.1}+e^{5.6}} \ $$
